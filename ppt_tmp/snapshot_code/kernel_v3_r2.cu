...
static std::string build_cutlass_so() {
    // Try dual-tile .so first (has both 64x128 + 128x128), then pre-built
    const char* paths[] = {
...
static void grow(int G){if(G<=g_mx)return;int n=std::max(G,64);
    ...
    cudaMalloc(&d_lA,n*sizeof(*d_lA));cudaMalloc(&d_lB,n*sizeof(*d_lB));g_mx=n;}
struct GemmArgs {
...
python3 -c "import ctypes; ctypes.CDLL('$SO_PATH'); print('Verification: OK')"
    ...
    const char* so_path = "/tmp/IFKernel_libcutlass_bw_gemm_dual.so";
...
static CutlassBwFn get_cutlass_bw_fn() {
    ...
    if (!tried) { tried = true;
        ...
        if (!so_path.empty()) {
            ...
            if (lib) {
                fn = (CutlassBwFn)dlsym(lib, "cutlass_blockwise_fp8_gemm");
                g_cutlass_fn_128 = (CutlassBwFn)dlsym(lib, "cutlass_blockwise_fp8_gemm_128");
            }
            else fprintf(stderr, "[CUTLASS] dlopen failed: %s\n", dlerror());
        }
    }
    return fn;
}
...
typedef cublasStatus_t (*fn_cublasSetAtomicsMode)(cublasHandle_t, cublasAtomicsMode_t);
typedef cublasStatus_t (*fn_cublasSetWorkspace)(cublasHandle_t, void*, size_t);
typedef cublasStatus_t (*fn_cublasGemmEx)(cublasHandle_t, cublasOperation_t, cublasOperation_t,
...
typedef cublasStatus_t (*fn_cublasGemmBatchedEx)(cublasHandle_t, cublasOperation_t, cublasOperation_t,
    int, int, int, const void*, const void* const[], cudaDataType, int,
    const void* const[], cudaDataType, int, const void*, void* const[], cudaDataType, int,
...
static void ensure_cublas_loaded() {
    ...
    if (!g_cublas_lib) { fprintf(stderr, "Failed to load libcublas: %s\n", dlerror()); return; }
    p_cublasCreate = (fn_cublasCreate)dlsym(g_cublas_lib, "cublasCreate_v2");
    p_cublasSetStream = (fn_cublasSetStream)dlsym(g_cublas_lib, "cublasSetStream_v2");
    p_cublasSetMathMode = (fn_cublasSetMathMode)dlsym(g_cublas_lib, "cublasSetMathMode");
    p_cublasSetAtomicsMode = (fn_cublasSetAtomicsMode)dlsym(g_cublas_lib, "cublasSetAtomicsMode");
    p_cublasSetWorkspace = (fn_cublasSetWorkspace)dlsym(g_cublas_lib, "cublasSetWorkspace_v2");
    p_cublasGemmEx = (fn_cublasGemmEx)dlsym(g_cublas_lib, "cublasGemmEx");
    p_cublasGemmBatchedEx = (fn_cublasGemmBatchedEx)dlsym(g_cublas_lib, "cublasGemmBatchedEx");
}
...
__global__ __launch_bounds__(256) void routing_kernel(
    ...
    if (warp_id < kNumGroups) {
        ...
        for (int offset = 16; offset > 0; offset >>= 1) {
            ...
            if (o_val > best1_val) { best1_val = o_val; best1_idx = o_idx; }
        }
        ...
        for (int offset = 16; offset > 0; offset >>= 1) {
            ...
            if (o_val > best2_val) { best2_val = o_val; }
        }
    ...
    for (int k = 0; k < kTopK; ++k) {
        ...
        int best_idx = tid;
        if (tid < kNumExpertsGlobal && !expert_used[tid] && group_kept[tid / (kNumExpertsGlobal / kNumGroups)]) {
            best_val = sb[tid];
        }
...
__global__ void scatter_local_assignments_kernel(
    ...
    int32_t* __restrict__ packed_tok,
    float* __restrict__ packed_w,
...
__global__ void gather_dequant_hidden_kernel(
...
__global__ void gather_dequant_hidden_fp16_kernel(
    const uint8_t* __restrict__ hidden_fp8,
    const float* __restrict__ hidden_scale,
    const int32_t* __restrict__ token_idx,
    int t, int tk,
...
__global__ void dequant_w13_batched_fp16_kernel(
    const uint8_t* __restrict__ w13_all_fp8,
    ...
    const int64_t expert_s13_base = static_cast<int64_t>(expert) * ((2 * kIntermediate) / kBlock) * (kHidden / kBlock);
...
__global__ void dequant_w2_batched_fp16_kernel(
    const uint8_t* __restrict__ w2_all_fp8,
    ...
    const int64_t expert_s2_base = static_cast<int64_t>(expert) * (kHidden / kBlock) * (kIntermediate / kBlock);
...
__global__ void swiglu_rowscale_fp16_batched_kernel(
    const __half* __restrict__ g1, int max_M,
    const int32_t* __restrict__ d_counts,
    __half* __restrict__ c_fp16, float* __restrict__ row_scale) {
...
__global__ void swiglu_to_fp8_kernel(
    const nv_bfloat16* __restrict__ g1_bf16, int max_M,
    const int32_t* __restrict__ d_counts,
    ...
    for (int j = 0; j < kEPT; j++) {
        int idx = j * kThreads + tid;
        int blk = idx / kBlock;
        c_fp8[out_base + idx] = __nv_cvt_float_to_fp8(vals[j] * smem_block_inv[blk], __NV_SATFINITE, __NV_E4M3);
    }
}
...
__global__ void gather_dequant_hidden_fp16_batched_v2_kernel(
    ...
    const __half2_raw r0 = __nv_cvt_fp8x2_to_halfraw2(static_cast<__nv_fp8x2_storage_t>(p0 & 0xffffu), __NV_E4M3);
    const __half2_raw r1 = __nv_cvt_fp8x2_to_halfraw2(static_cast<__nv_fp8x2_storage_t>(p0 >> 16u), __NV_E4M3);
    const __half2_raw r2 = __nv_cvt_fp8x2_to_halfraw2(static_cast<__nv_fp8x2_storage_t>(p1 & 0xffffu), __NV_E4M3);
    const __half2_raw r3 = __nv_cvt_fp8x2_to_halfraw2(static_cast<__nv_fp8x2_storage_t>(p1 >> 16u), __NV_E4M3);
...
__global__ void gather_fp8_and_scales_k(
...
__global__ __launch_bounds__(256, 8) void gather_fp8_and_scales_tight_k(
    ...
    while (lo < hi) { int mid = (lo + hi) >> 1; if (__ldg(row_offsets + mid + 1) <= global_row) lo = mid + 1; else hi = mid; }
    const int i = lo;
...
__global__ __launch_bounds__(256, 4) void pull_scatter_bf16_from_bf16_tight_kernel(
    ...
    const int32_t* __restrict__ le_to_rank, const int32_t* __restrict__ row_offsets,
    ...
    // kHidden=7168, 256 threads: use uint4 (8 bf16) for first 3 iters (6144 elems), uint2 (4 bf16) for last iter (1024 elems)
    constexpr int kThreads = 256;
    ...
    if (s_valid_count > 0) {
        ...
        for (int ki = 0; ki < s_valid_count; ++ki) {
            ...
            for (int iter = 0; iter < kIter8; iter++) {
                ...
                acc8[iter * 8 + 0] += f01.x * w; acc8[iter * 8 + 1] += f01.y * w;
                acc8[iter * 8 + 2] += f23.x * w; acc8[iter * 8 + 3] += f23.y * w;
                acc8[iter * 8 + 4] += f45.x * w; acc8[iter * 8 + 5] += f45.y * w;
                acc8[iter * 8 + 6] += f67.x * w; acc8[iter * 8 + 7] += f67.y * w;
            }
            // Narrower uint2 loads (8 bytes = 4 bf16) for remaining 1024 elements
            {
                ...
                const float2 f01 = __bfloat1622float2(bf2[0]);
                const float2 f23 = __bfloat1622float2(bf2[1]);
                acc4[0] += f01.x * w; acc4[1] += f01.y * w;
                acc4[2] += f23.x * w; acc4[3] += f23.y * w;
            }
        }
    }
    ...
    for (int iter = 0; iter < kIter8; iter++) {
        ...
        uint4 pk;
        pk.x = *reinterpret_cast<uint32_t*>(&o0); pk.y = *reinterpret_cast<uint32_t*>(&o1);
        pk.z = *reinterpret_cast<uint32_t*>(&o2); pk.w = *reinterpret_cast<uint32_t*>(&o3);
    ...
    // Write narrow (uint2) for last 1024 elements
    {
        ...
        uint2 pk; pk.x = *reinterpret_cast<uint32_t*>(&o0); pk.y = *reinterpret_cast<uint32_t*>(&o1);
...
__global__ __launch_bounds__(256, 4) void pull_scatter_bf16_kernel(
    ...
    const int32_t* __restrict__ packed_invrow, const int32_t* __restrict__ le_to_rank,
    int t, int max_M, int local_expert_offset,
    nv_bfloat16* __restrict__ out) {
    ...
    if (tok >= t) return;
    const int tid = threadIdx.x;
    int ranks[kTopK]; int rows[kTopK]; float scale_ws[kTopK]; int valid_count = 0;
    #pragma unroll
    for (int k = 0; k < kTopK; ++k) {
        const int global_k = tok * kTopK + k;
        ...
        ranks[valid_count] = rank; rows[valid_count] = row;
    ...
    for (int ki = 0; ki < valid_count; ++ki) {
        ...
        for (int iter = 0; iter < kIter; iter++) {
            ...
            acc[iter * 4 + 0] += f01.x * sw; acc[iter * 4 + 1] += f01.y * sw;
            acc[iter * 4 + 2] += f23.x * sw; acc[iter * 4 + 3] += f23.y * sw;
        }
    }
    const int64_t out_base = (int64_t)tok * kHidden;
    #pragma unroll
...
__global__ __launch_bounds__(256, 4) void pull_scatter_bf16_from_bf16_kernel(
    ...
    if (tok >= t) return;
    const int tid = threadIdx.x;
    int64_t in_bases[kTopK]; float weights[kTopK]; int valid_count = 0;
    #pragma unroll
    for (int k = 0; k < kTopK; ++k) {
        const int global_k = tok * kTopK + k;
        const int ge = __ldg(topk_idx + global_k);
    ...
    for (int ki = 0; ki < valid_count; ++ki) {
        ...
        for (int iter = 0; iter < kIter; iter++) {
            ...
            acc[iter * 4 + 0] += f01.x * w; acc[iter * 4 + 1] += f01.y * w;
            acc[iter * 4 + 2] += f23.x * w; acc[iter * 4 + 3] += f23.y * w;
        }
    }
    const int64_t out_base = (int64_t)tok * kHidden;
    #pragma unroll
...
__global__ void fused_bf16_swiglu_fp16_kernel(
    const nv_bfloat16* __restrict__ g1_bf16, int max_M,
    const int32_t* __restrict__ d_counts, __half* __restrict__ c_fp16,
    ...
    __shared__ float s_inv_scale;
    const int64_t g1_base = ((int64_t)expert * max_M + row) * (2 * kI);
    float local_vals[kElemsPerThread]; float local_max = 0.0f;
    constexpr int kPairsPerThread = kElemsPerThread / 2;
    for (int j = 0; j < kPairsPerThread; ++j) {
        const int i2 = j * kThreads + tid;
        nv_bfloat16 bf_x1[2], bf_x2[2];
        *reinterpret_cast<uint32_t*>(bf_x1) = *reinterpret_cast<const uint32_t*>(g1_bf16 + g1_base + i2 * 2);
        *reinterpret_cast<uint32_t*>(bf_x2) = *reinterpret_cast<const uint32_t*>(g1_bf16 + g1_base + kI + i2 * 2);
        ...
        local_vals[j * 2] = v0; local_vals[j * 2 + 1] = v1;
        local_max = fmaxf(local_max, fmaxf(fabsf(v0), fabsf(v1)));
    }
    for (int s = 16; s > 0; s >>= 1)
        local_max = fmaxf(local_max, __shfl_down_sync(0xffffffffu, local_max, s));
...
__global__ void exclusive_scan_32_kernel(
    const int32_t* __restrict__ counts, int32_t* __restrict__ offsets,
    ...
    // Layout: [row_offsets(33)] [expert_ids(32)] [active_counts(32)] [base_offsets(32)] [le_to_rank(32)]
    if (combined_out != nullptr) {
...
// Layout in combined_out: [row_offsets(33)] [expert_ids(32)] [active_counts(32)] [base_offsets(32)] [le_to_rank(32)]
...
struct CudaBuf {
    ...
    void alloc(size_t n) {
        ...
        if (e != cudaSuccess) {
            ...
            ptr = nullptr; bytes = 0; return;
        }
        bytes = n; cudaMemset(ptr, 0, n);
    }
    void free_buf() { if (ptr) { cudaFree(ptr); ptr = nullptr; bytes = 0; } }
    ~CudaBuf() { free_buf(); }
...
int parse_env_int(const char* name, int default_value) {
    ...
    if (end == v || *end != 0 || parsed <= 0 || parsed > std::numeric_limits<int>::max()) return default_value;
...
KernelWorkspace& get_workspace(int device_index, int chunk_cap) {
    ...
    if (need_realloc) {
        ws.device_index = device_index; ws.chunk_cap = chunk_cap;
    ...
    if (!ws.pinned_h_counts) CUDA_CHECK(cudaMallocHost(&ws.pinned_h_counts, sizeof(int32_t) * kNumLocalExperts));
    if (!ws.pinned_dequant_ids) CUDA_CHECK(cudaMallocHost(&ws.pinned_dequant_ids, sizeof(int32_t) * kNumLocalExperts));
    if (!ws.pinned_sentinel) {
        CUDA_CHECK(cudaHostAlloc(&ws.pinned_sentinel, sizeof(int32_t), cudaHostAllocMapped));
        CUDA_CHECK(cudaHostGetDevicePointer(reinterpret_cast<void**>(&ws.pinned_sentinel_dev), ws.pinned_sentinel, 0));
    ...
    if (!ws.mapped_total_rows) {
        ...
        CUDA_CHECK(cudaHostGetDevicePointer(reinterpret_cast<void**>(&ws.mapped_total_rows_dev), ws.mapped_total_rows, 0));
        ws.mapped_total_rows[0] = 0;
    }
    // pinned_combined layout: [row_offsets (max 33)] [expert_ids (max 32)] [active_counts (max 32)] [base_offsets (max 32)] [le_to_rank (32)]
    if (!ws.pinned_combined) CUDA_CHECK(cudaMallocHost(&ws.pinned_combined, (5 * kNumLocalExperts + 1) * sizeof(int32_t)));
    if (!ws.d_batch_ptr_buf.defined()) {
        constexpr size_t kBatchBufBytes = static_cast<size_t>(kNumLocalExperts) * (6 * sizeof(void*) + 3 * sizeof(int32_t));
...
cublasHandle_t get_cublas_handle(cudaStream_t stream) {
    ...
    if (!handle) {
        ...
        const cublasMath_t math_mode = env_is_fp32_math() ? CUBLAS_DEFAULT_MATH : CUBLAS_TF32_TENSOR_OP_MATH;
...
void cublas_gemm_loop_host(
    ...
    void* const* h_A, cudaDataType Atype, int lda,
    void* const* h_B, cudaDataType Btype, int ldb,
    const float* beta, void* const* h_C, cudaDataType Ctype, int ldc,
    int start, int count, cublasComputeType_t computeType, cublasGemmAlgo_t algo)
{
    for (int i = start; i < start + count; i++)
        CUBLAS_CHECK(p_cublasGemmEx(handle, transa, transb, m, n, k, alpha,
            h_A[i], Atype, lda, h_B[i], Btype, ldb, beta, h_C[i], Ctype, ldc,
            computeType, algo));
}
...
tvm::ffi::Tensor kernel(
    ...
    if (__builtin_expect(tl_first_call, 0)) { init_fp8_lut_once(); configure_kernel_cache_once(); tl_first_call = false; }
    ...
    if (__builtin_expect(!ec.initialized, 0)) {
        ...
        if (ec.gemm_n_align != 1 && ec.gemm_n_align != 2 && ec.gemm_n_align != 4 &&
            ec.gemm_n_align != 8 && ec.gemm_n_align != 16 && ec.gemm_n_align != 32) ec.gemm_n_align = 1;
    ...
    const int tk_chunk = (t >= ec.longseq_threshold) ? ec.long_chunk : ((t >= ec.short_chunk) ? ec.base_chunk : ec.short_chunk);
    ...
    const nv_bfloat16* routing_bias_ptr_bf = reinterpret_cast<const nv_bfloat16*>(routing_bias.data_ptr());
    ...
    const bool pointer_unchanged = ws.signature_valid &&
        ws.cached_w13_ptr == gemm1_weights.data_ptr() && ws.cached_s13_ptr == gemm1_weights_scale.data_ptr() &&
        ws.cached_w2_ptr == gemm2_weights.data_ptr() && ws.cached_s2_ptr == gemm2_weights_scale.data_ptr();
    ...
    tvm::ffi::Tensor out_bf16 = alloc_tensor(tvm::ffi::ShapeView(out_shape, 2), bf16_dtype, dl_device);
    ...
    if (gpu_planner_path) {
        ...
        int32_t* rti = topk_idx_ptr; float* rtw = topk_w_ptr; int32_t* rc = counts_ptr;
        cudaLaunchKernelEx(&s_rs_pss_lc, routing_kernel, rl, rb, rt, rs, rleo, rti, rtw, rc);
    } else {
        routing_kernel<<<t, threads, 0, stream>>>(routing_logits_ptr_f, routing_bias_ptr_bf, t,
            static_cast<float>(routed_scaling_factor), static_cast<int>(local_expert_offset),
            topk_idx_ptr, topk_w_ptr, counts_ptr);
    ...
    int32_t* scan_combined_out = (gpu_planner_path && ws.row_offsets_dev.defined())
        ? ws.row_offsets_dev.as<int32_t>() : nullptr;
    if (gpu_planner_path) {
        s_rs_pss_lc.gridDim = div_up(t * kTopK, threads);
        s_rs_pss_lc.stream = stream;
        const int32_t* sti = topk_idx_ptr; const float* stw = topk_w_ptr;
        int st = t; int sleo = static_cast<int>(local_expert_offset);
        const int32_t* sc = counts_ptr;
        int32_t* so = offsets_ptr; int32_t* scur = cursors_ptr;
        int32_t* spt = packed_tok_ptr; float* spw = packed_w_ptr; int32_t* spi = packed_invrow_ptr;
        int32_t* sco = scan_combined_out; int32_t* smt = nullptr;
        cudaLaunchKernelEx(&s_rs_pss_lc, scatter_with_scan_kernel,
            sti, stw, st, sleo, sc, so, scur, spt, spw, spi, sco, smt);
    } else {
        scatter_with_scan_kernel<<<div_up(t * kTopK, threads), threads, 0, stream>>>(
            ...
            counts_ptr, offsets_ptr, cursors_ptr, packed_tok_ptr, packed_w_ptr, packed_invrow_ptr,
            scan_combined_out, nullptr);
        CUDA_CHECK(cudaGetLastError());
    }
    ...
    if (gpu_planner_path) {
        ...
        // Pre-allocate fp8 buffers for max possible rows (t * kTopK for safety)
        const int max_possible_rows = t * kTopK;
        if (max_possible_rows > ws.max_fp8_rows || !ws.fp8_buf.defined()) {
            ...
            ws.fp8_c_buf.alloc(static_cast<size_t>(max_possible_rows) * kIntermediate);
        ...
        // Use tighter grid estimate (t*3) to reduce empty block launches
        // Kernels have early-exit bounds checking via row_offsets[active_count]
        total_local_assignments = t * 3;
        ...
        CUDA_CHECK(cudaMemcpyAsync(ws.pinned_h_counts, counts_ptr,
            sizeof(int32_t) * kNumLocalExperts, cudaMemcpyDeviceToHost, sync_stream));
        if (!pointer_unchanged && gemm1_weights.data_ptr() != nullptr && sig_nbytes >= 16)
            CUDA_CHECK(cudaMemcpyAsync(ws.pinned_sig_buf, gemm1_weights.data_ptr(), 16, cudaMemcpyDeviceToHost, sync_stream));
        ...
        if (!pointer_unchanged) {
            if (gemm1_weights.data_ptr() != nullptr && sig_nbytes >= 16) {
                ...
                for (int i = 0; i < 16; ++i) { h1 ^= s[i]; h1 *= 1099511628211ull; }
                h1 ^= sig_nbytes; h1 *= 1099511628211ull;
                uint64_t h2 = 1099511628211ull;
                for (int i = 16; i > 0; --i) { h2 ^= s[i-1]; h2 *= 1469598103934665603ull; }
                ...
                ws.sig_w13 = cur_sig;
            } else { weights_changed = !ws.signature_valid; }
            ws.signature_valid = true;
            ws.cached_w13_ptr = gemm1_weights.data_ptr(); ws.cached_s13_ptr = gemm1_weights_scale.data_ptr();
            ws.cached_w2_ptr = gemm2_weights.data_ptr(); ws.cached_s2_ptr = gemm2_weights_scale.data_ptr();
        }
        if (weights_changed) { ws.dequant_ready.fill(0); ws.w2_dequant_ready.fill(0); }
        for (int le = 0; le < kNumLocalExperts; ++le) { h_offsets[le] = total_local_assignments; total_local_assignments += ws.pinned_h_counts[le]; }
        for (int le = 0; le < kNumLocalExperts; ++le) if (ws.pinned_h_counts[le] > 0) active_experts[active_count++] = le;
    }
    ...
    if (!gpu_planner_path) {
        ...
        std::array<int32_t, kNumLocalExperts> dequant_experts{}; int dequant_count = 0;
        for (int i = 0; i < active_count; ++i) { const int le = active_experts[i]; if (!ws.dequant_ready[le]) dequant_experts[dequant_count++] = static_cast<int32_t>(le); }
        if (dequant_count > 0 && !cutlass_bw) {
            CUDA_CHECK(cudaMemcpy(ws.dequant_ids.as<int32_t>(), dequant_experts.data(), sizeof(int32_t) * dequant_count, cudaMemcpyHostToDevice));
            dim3 w13_grid(kHidden / kBlock, (2 * kIntermediate) / kBlock, static_cast<unsigned int>(dequant_count));
            dequant_w13_batched_fp16_kernel<<<w13_grid, threads, 0, stream>>>(w13_all_ptr, s13_all_ptr, ws.dequant_ids.as<int32_t>(), ws.w13_all.as<__half>());
            CUDA_CHECK(cudaGetLastError());
            dim3 w2_grid(kIntermediate / kBlock, kHidden / kBlock, static_cast<unsigned int>(dequant_count));
            dequant_w2_batched_fp16_kernel<<<w2_grid, threads, 0, stream>>>(w2_all_ptr, s2_all_ptr, ws.dequant_ids.as<int32_t>(), ws.w2_all.as<__half>());
            CUDA_CHECK(cudaGetLastError());
    ...
    if (!gpu_planner_path) {
        int max_M_raw = 0;
        for (int i = 0; i < active_count; ++i) max_M_raw = std::max(max_M_raw, (int)ws.pinned_h_counts[active_experts[i]]);
    ...
    const __half* b_c_fp16_ptr = ws.b_c_fp16_all.defined() ? ws.b_c_fp16_all.as<const __half>() : nullptr;
    ...
    if (!cutlass_bw) {
        bool ptrs_stale = ws.ptr_cache_active_count != active_count || ws.ptr_cache_max_M != max_M ||
            ws.ptr_cache_w13_data != ws.w13_all.ptr || ws.ptr_cache_w2_data != ws.w2_all.ptr || ws.ptr_cache_ba_data != ws.b_a_all.ptr;
        if (!ptrs_stale) {
            for (int i = 0; i < active_count; ++i) {
                if (ws.ptr_cache_experts[i] != (int32_t)active_experts[i] || ws.ptr_cache_counts[i] != (int32_t)ws.pinned_h_counts[active_experts[i]]) { ptrs_stale = true; break; }
            }
        }
        if (ptrs_stale) {
            ...
            for (int i = 0; i < active_count; ++i) {
                ...
                hp[2*stride+i] = (void*)(b_g1_ptr + static_cast<int64_t>(i) * max_M * 2 * kIntermediate);
                hp[3*stride+i] = (void*)(w2_all_half + static_cast<int64_t>(le) * w2_elems);
                hp[4*stride+i] = (void*)(b_c_fp16_ptr + static_cast<int64_t>(i) * max_M * kIntermediate);
            ...
            for (int i = 0; i < active_count; ++i) { hc[i] = ws.pinned_h_counts[active_experts[i]]; hbo[i] = h_offsets[active_experts[i]]; }
            for (int le = 0; le < kNumLocalExperts; ++le) hltr[le] = -1;
            for (int i = 0; i < active_count; ++i) hltr[active_experts[i]] = i;
            CUDA_CHECK(cudaMemcpyAsync(ws.d_batch_ptr_buf.ptr, ws.pinned_batch_ptrs,
                static_cast<size_t>(kNumLocalExperts) * (6 * sizeof(void*) + 3 * sizeof(int32_t)), cudaMemcpyHostToDevice, stream));
            ws.ptr_cache_active_count = active_count; ws.ptr_cache_max_M = max_M;
            ws.ptr_cache_w13_data = ws.w13_all.ptr; ws.ptr_cache_w2_data = ws.w2_all.ptr; ws.ptr_cache_ba_data = ws.b_a_all.ptr;
            for (int i = 0; i < active_count; ++i) { ws.ptr_cache_experts[i] = active_experts[i]; ws.ptr_cache_counts[i] = ws.pinned_h_counts[active_experts[i]]; }
        }
    }
    const size_t stride_d = static_cast<size_t>(kNumLocalExperts);
    uint8_t* d_bp = ws.d_batch_ptr_buf.as<uint8_t>();
    const void* const* d_A1 = reinterpret_cast<const void* const*>(d_bp + 0*stride_d*sizeof(void*));
    const void* const* d_B1 = reinterpret_cast<const void* const*>(d_bp + 1*stride_d*sizeof(void*));
    void* const* d_C1 = reinterpret_cast<void* const*>(d_bp + 2*stride_d*sizeof(void*));
    const void* const* d_A2 = reinterpret_cast<const void* const*>(d_bp + 3*stride_d*sizeof(void*));
    const void* const* d_B2 = reinterpret_cast<const void* const*>(d_bp + 4*stride_d*sizeof(void*));
    ...
    if (!cutlass_bw && active_count > 0) {
        ...
        gather_dequant_hidden_fp16_batched_v2_kernel<<<dim3(gather_blocks_x, active_count), 256, 0, stream>>>(
            hidden_ptr, hidden_scale_ptr, packed_tok_ptr, d_base_offsets, d_active_counts, t, max_M, const_cast<__half*>(b_a_ptr));
        CUDA_CHECK(cudaGetLastError());
    }

    const bool pipeline_swiglu = !cutlass_bw && ec.pipeline_swiglu_enabled && (t >= ec.longseq_threshold);
    ...
    // Cached launch config with programmatic stream serialization (reused for gather/swiglu/pull_scatter)
    ...
    if (cutlass_bw && active_count > 0) {
        if (gpu_planner_path) {
            ...
            if (!ws.fp8_buf.defined() || ws.fp8_buf.bytes < static_cast<size_t>(rows) * kHidden) {
                ws.fp8_buf.alloc(rows * kHidden); ws.sfa_buf.alloc(rows * Hb * 4);
                ws.bf16_buf.alloc(rows * 2 * kIntermediate * 2);
                ws.row_offsets_dev.alloc((5 * kNumLocalExperts + 1) * 4);
                ws.cutlass_static_ready = false;
                ws.fp8_c_buf.alloc(rows * kIntermediate); ws.sfa_c_buf.alloc(rows * Ib * 4);
                ws.bf16_g2_buf.alloc(rows * kHidden * 2);
            }
            const int ac = active_count;
            int off = 0;
            memcpy(ws.pinned_combined + off, h_row_offsets, (ac+1)*4); off += ac+1;
            for (int i=0;i<ac;i++) ws.pinned_combined[off+i]=active_experts[i]; off += ac;
            for (int i=0;i<ac;i++) ws.pinned_combined[off+i]=ws.pinned_h_counts[active_experts[i]]; off += ac;
            for (int i=0;i<ac;i++) ws.pinned_combined[off+i]=h_offsets[active_experts[i]]; off += ac;
            ...
            cudaMemcpyAsync(ws.row_offsets_dev.as<int32_t>(), ws.pinned_combined, total_combined*4, cudaMemcpyHostToDevice, stream);
            int32_t* rod = ws.row_offsets_dev.as<int32_t>();
            d_eids = rod + (ac + 1);
        ...
        if (total_tight_rows > 0) {
        {
            ...
            const uint8_t* g_h = hidden_ptr; const float* g_hs = hidden_scale_ptr;
            const int32_t* g_pt = packed_tok_ptr; const int32_t* g_bo = d_base_offsets;
            const int32_t* g_dc = d_active_counts; const int32_t* g_ro = ws.row_offsets_dev.as<int32_t>();
            int g_t = t; int g_ac = gpu_planner_path ? kNumLocalExperts : active_count;
            uint8_t* g_fp8 = ws.fp8_buf.as<uint8_t>(); float* g_sfa = ws.sfa_buf.as<float>();
            cudaLaunchKernelEx(&s_pss_lc, gather_fp8_and_scales_tight_k,
                g_h, g_hs, g_pt, g_bo, g_dc, g_ro, g_t, g_ac, g_fp8, g_sfa);
        }
        if (!gpu_planner_path) { CUDA_CHECK(cudaGetLastError()); }
        const int cutlass_num_groups = gpu_planner_path ? kNumLocalExperts : active_count;
        CutlassBwArgs cargs; cargs.num_groups=cutlass_num_groups; cargs.N=2*kIntermediate; cargs.K=kHidden;
        cargs.A=ws.fp8_buf.ptr; cargs.B=const_cast<uint8_t*>(w13_all_ptr); cargs.D=ws.bf16_buf.ptr;
        cargs.SFA=ws.sfa_buf.ptr; cargs.SFB=const_cast<float*>(s13_all_ptr);
        cargs.m_indptr=ws.row_offsets_dev.as<int32_t>(); cargs.expert_ids=d_eids;
        CutlassBwFn gemm1_fn = cutlass_bw;
        int max_M_estimate = gpu_planner_path ? (total_tight_rows / kNumLocalExperts + 1) : max_M;
        if (max_M_estimate > 256 && g_cutlass_fn_128) gemm1_fn = g_cutlass_fn_128;
...

    {
        ...
        if (pipeline_swiglu && !pipeline_init) {
            ...
            for (int i = 0; i < kNumLocalExperts; ++i) CUDA_CHECK(cudaEventCreateWithFlags(&bkt_events[i], cudaEventDisableTiming));
        ...
        for (int b = 0; b < gemm_bucket_count; ++b) {
            ...
            cublas_gemm_loop_host(handle, CUBLAS_OP_T, CUBLAS_OP_N,
                2*kIntermediate, bucket.rounded_m, kHidden, &alpha,
                h_A1, CUDA_R_16F, kHidden, h_B1, CUDA_R_16F, kHidden, &beta0,
                h_C1, CUDA_R_16F, 2*kIntermediate, bucket.start, bucket.end - bucket.start, gemm1_compute_type, gemm1_algo);
            if (pipeline_swiglu) {
                CUDA_CHECK(cudaEventRecord(bkt_events[b], stream));
                CUDA_CHECK(cudaStreamWaitEvent(aux_stream, bkt_events[b], 0));
                swiglu_rowscale_fp16_batched_kernel<<<dim3(max_M, bucket.end - bucket.start), 256, 0, aux_stream>>>(
                    b_g1_ptr + (int64_t)bucket.start * max_M * 2 * kIntermediate, max_M, d_active_counts + bucket.start,
        ...
        if (pipeline_swiglu) {
            ...
            for (int b = 0; b < gemm_bucket_count; ++b) {
                const GemmBucketRange& bucket = gemm_buckets[b];
                swiglu_rowscale_fp16_batched_kernel<<<dim3(max_M, bucket.end - bucket.start), 256, 0, stream>>>(
                    b_g1_ptr + (int64_t)bucket.start * max_M * 2 * kIntermediate, max_M, d_active_counts + bucket.start,
                    const_cast<__half*>(b_c_fp16_ptr) + (int64_t)bucket.start * max_M * kIntermediate,
                    b_c_scale_ptr + (int64_t)bucket.start * max_M);
                CUDA_CHECK(cudaGetLastError());
    ...
    if (cutlass_done && cutlass_bw) {
        {
            ...
            cudaLaunchKernelEx(&s_pss_lc, swiglu_to_fp8_tight_kernel,
                sw_in, sw_ro, sw_ac, sw_fp8, sw_sfa, sw_rs);
        }
        if (!gpu_planner_path) { CUDA_CHECK(cudaGetLastError()); }
        const int cutlass_num_groups = gpu_planner_path ? kNumLocalExperts : active_count;
        CutlassBwArgs cargs2; cargs2.num_groups=cutlass_num_groups; cargs2.N=kHidden; cargs2.K=kIntermediate;
        cargs2.A=ws.fp8_c_buf.ptr; cargs2.B=const_cast<uint8_t*>(w2_all_ptr); cargs2.D=ws.bf16_g2_buf.ptr;
        cargs2.SFA=ws.sfa_c_buf.ptr; cargs2.SFB=const_cast<float*>(s2_all_ptr);
        cargs2.m_indptr=ws.row_offsets_dev.as<int32_t>(); cargs2.expert_ids=d_eids;
        CutlassBwFn gemm2_fn = cutlass_bw;
        int max_M_est2 = gpu_planner_path ? (total_tight_rows / kNumLocalExperts + 1) : max_M;
        if (max_M_est2 > 256 && g_cutlass_fn_128) gemm2_fn = g_cutlass_fn_128;
        int ret2 = gemm2_fn(&cargs2, stream);
        if (ret2 != 0 && gemm2_fn != cutlass_bw) {
            ret2 = cutlass_bw(&cargs2, stream);
        }
        if (ret2 == 0) { gemm2_bf16_out = ws.bf16_g2_buf.as<const nv_bfloat16>(); cutlass_gemm2_done = true; }
    }
...

    {
        ...
        for (int b = 0; b < gemm_bucket_count; ++b) {
            const GemmBucketRange& bucket = gemm_buckets[b];
            cublas_gemm_loop_host(handle, CUBLAS_OP_T, CUBLAS_OP_N,
                kHidden, bucket.rounded_m, kIntermediate, &alpha,
                h_A2, CUDA_R_16F, kIntermediate, h_B2, CUDA_R_16F, kIntermediate, &beta0,
                h_C2, CUDA_R_16F, kHidden, bucket.start, bucket.end - bucket.start, gemm2_compute_type, gemm2_algo);
        }
    }
    ...
    if (t > 0) {
        ...
        if (gemm2_bf16_out) {
            {
                ...
                cudaLaunchKernelEx(&s_pss_lc, pull_scatter_bf16_from_bf16_tight_kernel,
                    ps_bo, ps_ti, ps_tw, ps_pi, ps_lr, ps_ro, ps_t, ps_leo, ps_out, ps_ctz, ps_ctzn);
            }
            ws.counts_zeroed_by_pull_scatter = true;
        } else {
            pull_scatter_bf16_kernel<<<t, 256, 0, stream>>>(b_o_ptr, b_c_scale_ptr,
                topk_idx_ptr, topk_w_ptr, packed_invrow_ptr, d_le_to_rank, t, max_M,
                static_cast<int>(local_expert_offset), out_ptr);
        ...
        CUDA_CHECK(cudaMemsetAsync(out_bf16.data_ptr(), 0, static_cast<size_t>(t) * kHidden * 2, stream));
